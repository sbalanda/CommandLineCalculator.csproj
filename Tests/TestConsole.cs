...
namespace CommandLineCalculator.Tests
{
    public class TestConsole : UserConsole
    {
        ...
        {
            Read,// ввод через консоль
            Write // вывод на консоль
        }
        ...
        public override string ReadLine()
        {
            if (position >= schedule.Length)
            {
                throw new TestException(
                    "Attempt to read at end", // попытка читать, когда конец
        ...
        public override void WriteLine(string content)
        {
            if (position >= schedule.Length)
            {
                //Попытка написать в конце
                throw new TestException("Attempt to write at end", TestException.ExceptionType.WrongWrite);
            }
            ...
            if (action != Action.Write)
            {
                ...
                // Попытка написать, когда ожидается чтение
                throw new TestException($"Attempt to write when read expected.\n{state}", TestException.ExceptionType.WrongWrite);
            }

            if (!string.Equals(value, content, StringComparison.InvariantCulture))
            {
                var state = PrintError($"WRITE: {content}");
                // Попытка написать не то, что ожидалось
                throw new TestException($"Attempt to write not what expected.\n{state}", TestException.ExceptionType.WrongWrite);
            }
